...
def get_plain_chat_prompt(question, tables=None):
    function_info = "\n".join(FUNCTION_DESCRIPTION.values())

    function_prompt = """
You can use the following functions:
"""
    data_prompt = get_db_info_prompt(engine, simple=True, example=False, tables=tables)
...
def get_plain_chat(question: str, tables=None):
    prompt = get_plain_chat_prompt(question, tables)
    ans = call_llm(prompt, llm)
    return ans.content


def get_plain_chat_stream(question: str, tables=None):
    prompt = get_plain_chat_prompt(question, tables)
